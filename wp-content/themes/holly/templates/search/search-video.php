        ...
        <div class="grid-item-inner third-inner<?php $position % $per_row == 0 ? ' third-no-right-border' : '' ?>">
            <article id="post-<?php $post->ID ?>" <?php post_class(); ?>>
                <header>
                    ...
                    <h2 class="video-title blackmaroon"><a href="<?php get_permalink() ?>" title="View <?php esc_attr(get_the_title()) ?>"><? the_title() ?></a></h2>
...
<?php if($block['is_main']): ?>
    <div class="search-results-section-bottom">
        ...
        $args['search_type'] = $block['type'];
        ?>
        <div class="results-more more"><a href="<?phpapply_filters('ice-search-link', '', $args);?>">see all videos &raquo;</a></div>
    </div>
