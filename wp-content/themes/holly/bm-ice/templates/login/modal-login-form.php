...
<div id="login-page">
	...
	<h2 class="sign-in-title">Sign in to <span class="sign-in-its-name"><?phpget_bloginfo('name');?></span> on Celebuzz</h2>
	<table><tbody>
	<tr><td class="sign-in-form-left">
        <?php if(isset($_REQUEST['fb_login_error'])){ ?>
            <span id="ice-login-status" class="invalid">
                <?phpapply_filters('ice_fb_get_message_by_error_code', '', $_REQUEST['fb_login_error']);?>
            </span>
        <?php }else { ?>
	...
	<div id="login-links">
	Not a member? <a class="join" href="<?phpsite_url(ICE_REGISTER_SLUG);?>">Join Now</a> / <a href="<?phpsite_url('/wp-login.php?action=lostpassword');?>">Forgot your password?</a>
	</div>
