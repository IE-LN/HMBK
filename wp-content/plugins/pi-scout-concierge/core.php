	...
	public static function mb_edit_settings() {
		...
		<form method="post" method="/wp-admin/options-general.php?page=pi-scout-concierge">
			...
			<div style="margin: 10px">
				Horizon Domain
				<input type="text" name="<?= self::$opt_pre ?>horizont_domain" value="<?= self::$settings['horizont_domain']?>" /><br/>
                Show concierge
                <input type="hidden" name="<?= self::$opt_pre ?>show_concierge" value="0"  />
                <input type="checkbox" name="<?= self::$opt_pre ?>show_concierge" value="1" <?php checked(1, !empty( self::$settings['show_concierge'])); ?> /><br/>
				<br/>
				...
				<input type="submit" name="<?= self::$opt_pre ?>save_bt" value="Save" />
			</div>
		</form>
		<?php
	}
